...
When an app is source-control-backed, the repository becomes the source of truth
for that app's source. The app page still renders the saved built artifact/cache
for speed; source control is used for explicit publish/sync, workspace
source-storage sync, Available Apps install/update, and source restore after a
live worker session is gone. See [Source Control](/source-control) for the full
...
- [Source Control](/source-control): source-control-backed app source storage, Available Apps, auto-versioning, and restore boundaries
